...
- [ ] ECS、RDS、VPC Flow Log 用の Log Groups が作成される
- [ ] Dev 環境で 3 日間のログ保持期間が設定される
- [ ] Prod 環境で 30 日間のログ保持期間が設定される
- [ ] Prod 環境で S3 Glacier への Export 設定が構成される
- [ ] 環境別パラメータによる動的設定が可能である
- [ ] 全てのユニットテストが通過する
